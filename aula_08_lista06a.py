...
class Cliente:
    ...
    @staticmethod
    def menu():
        print("1-Inserir, 2-Listar, 3-Atualizar, 4-Excluir, 5-Pesquisar,  9-Fim")
        return int(input("Informe uma opção: "))
    @classmethod
    def salvar(csl):
        arquivo = open ("clientes,json", mode = "w")
        json.dump(csl.__objetos, arquivo,default = Cliente.to_json, indent = 2)
        arquivo.close()
    ...
    def inserir(cls):
        ...
        x = Cliente(id, nome, fone, email)
        cls.__objetos.append(x)

    @classmethod
    def listar(cls):                
        for x in cls.__objetos: print(x, x())

    @classmethod
    def atualizar(cls):
        for x in cls.__objetos: print(x)
        id = int(input("Informe o id do cliente ser atualizado: "))
